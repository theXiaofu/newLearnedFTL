...
//缓存大小
static Cache* init_cache(int cache_size,int write_cache_size){
    ...
    //写缓存空间
    write_cache->write_table = (Table*)cache;
...
static inline void print_seglru(struct ssd*ssd,int line)
{
    ...
    {
        if(seg_LRU[i].pos_entry_number !=INVALID_POS_ENTRY)
        {
            uint32_t header;
            ...
            if(header!=i)
            {
                //输出整个read cache 的space
                printf("read cache space num: %d",read_cache->space_num);
                for(int j = 0;j<read_cache->space_num;j++)
                {
                    //输出space的所有参数
                    //输出space的各个参数
                ...
                printf("error: line: %d seg_LRU[%d]  reverse indx: %d \n",line,i,header);
                printf("error: line: %d seg_LRU[%d]  seg_Lru indx: %d \n",line,i,seg_LRU[i].pos_entry_number);
...
static inline void print_space_seg(struct ssd* ssd,int line){
    ...
    Write_Cache *write_cache = ftl_map->cache->write_cache;
    Read_Cache *read_cache = ftl_map->cache->read_cache;
    for(int i = 0;i<write_cache->write_point;++i)
    {
        int pos = write_cache->write_table[i].table_head;
        if(i!=seg_LRU[pos].pos_entry_number||seg_LRU[pos].seg_num!=WRITE_CACHE_SPACE)
        {
            printf("error: line: %d \n",line);
            //打印seg_LRU和writecache的信息
            printf("write_cache[%d].table_head = %d\n",i,write_cache->write_table[i].table_head);
            printf("seg_LRU[%d].pos_entry_number = %d\n",pos,seg_LRU[pos].pos_entry_number);
            printf("seg_LRU[%d].seg_num = %d\n",pos,seg_LRU[pos].seg_num);
            exit(0);
        }
    ...
    for(int i = 0;i<read_cache->space_num;++i)
    {
        if(read_cache->read_cache_space[i].num*read_cache->read_cache_space[i].size!=read_cache->read_cache_space[i].end-read_cache->read_cache_space[i].st)
        {
            printf("error: line: %d \n",line);
            printf("read_cache->read_cache_space[%d].num = %d\n",i,read_cache->read_cache_space[i].num);
            printf("read_cache->read_cache_space[%d].end = %d\n",i,read_cache->read_cache_space[i].end);
            printf("read_cache->read_cache_space[%d].st = %d\n",i,read_cache->read_cache_space[i].st);
            printf("read_cache->read_cache_space[%d].size = %d\n",i,read_cache->read_cache_space[i].size);
            exit(0);
        }
        
        for(int j = 0;j<read_cache->read_cache_space[i].num;++j)
        {
            int pos = *(int*)(read_cache->read_cache + read_cache->read_cache_space[i].st+j*read_cache->read_cache_space[i].size);
            if(seg_LRU[pos].pos_entry_number != (read_cache->read_cache_space[i]).st+j*read_cache->read_cache_space[i].size)
            {
                printf("error: line: %d \n",line);
                printf("pos = %d,seg_LRU[pos].pos_entry_number = %d\n",pos,seg_LRU[pos].pos_entry_number);
                printf("read_cache->read_cache_space[%d].st = %d,read_cache->read_cache_space[%d].size = %d\n",i,read_cache->read_cache_space[i].st,i,read_cache->read_cache_space[i].size);
                printf("j: %d\n",j);
                exit(0);
            }
        }
    }
    int read_head = ssd->ftl_map->read_cache_LRU_head;
    int nex = seg_LRU[read_head].nex;
    // while(nex!=read_head){

    //     if(seg_LRU[nex].pos_entry_number==INVALID_POS_ENTRY||seg_LRU[nex].seg_num==WRITE_CACHE_SPACE){
    //         printf("error: line: %d \n",line);
    //         printf("seg_LRU[%d].pos_entry_number = %d\n",nex,seg_LRU[nex].pos_entry_number);
    //         printf("seg_LRU[%d].seg_num = %d\n",nex,seg_LRU[nex].seg_num);
    //         exit(0);
    //     }
    //     nex=seg_LRU[nex].nex;
    // }
...
*/
/*
static inline void print_read_space(struct ssd* ssd,int line){ 
...
static void ssd_init_params(struct ssdparams *spp)
{
    ...
    spp->blks_per_pl = 1024; /*default 256，8GB */
...
{
    int wwp_pos = tvpn/(ssd->sp.pgs_per_line);
    //struct ssdparams *spp = &ssd->sp;
     ...
     if(i>0)
     {
        ...
        while(i<space_idx)
        {
            ...
            for(int j = 0;j<tmp;++j)
            {
                ...
                seg_lru[header].pos_entry_number = nex_st;
...
// for(int i = space_idx;i>0;--i)
// {
//     printf(" nead_size[%d] = %d\n", i,nead_size[i]);
// }

//print_read_space(ssd,__LINE__);

//如果空间不足那么使用LRU驱逐一直驱逐直到空间足够
while(TRUE)
{
    int32_t evict_size = read_cache->max_evict_size;
    ...
    while(evict_size>0)
    {
        ...
        if(victim_seg_idx==ftl_map->read_cache_LRU_head){
            //print_read_space(ssd,__LINE__);
            //printf("max_evict_size = %d\n",read_cache->max_evict_size);
            exit(0);
        }
        //获得子空间号
        uint32_t victim_space_idx = read_cache->segsize2space[seg_lru[victim_seg_idx].seg_num];
        //printf("victim_seg_idx = %d  victim_space_idx:%d\n",victim_seg_idx,victim_space_idx);
        ...
        if(victim_space_idx>space_idx)
        {
            //printf("%d\n",__LINE__);
            // printf("movebegin:%d\n",move_begin);
            // printf("victim _space_id:%d\n",victim_space_idx);
            // printf("header:%d\n",header);
            // printf("seg_index:%d\n",seg_idx);
            //print_read_space(ssd,__LINE__);
            //在space中删除 如果不在左端将左端的seg移到这个位置
            ...
            // if((int)read_space[victim_space_idx].num<0)
            // {
            //     printf("victim_space_idx:%d\n",victim_space_idx);
            //     printf("victim_seg_idx:%d\n",victim_seg_idx);
            //     exit(0);
            // }
            //print_space_seg(ssd,__LINE__);
            //printf("%d\n",__LINE__);
...
static void insert_table_to_write_table(struct ssd*ssd,Table* table_st)
{
    ...
    Write_Cache* write_cache = ftl_map->cache->write_cache;
    Seg_LRU* seg_lru = ftl_map->seg_LRU;
    uint32_t seg_idx = table_st->table_head;
    ...
    if(write_cache->write_point==write_cache->write_table_capacity)
    {
       ...
       //从lru中删除
        ...
        seg_lru[victim_seg_idx].pos_entry_number = INVALID_POS_ENTRY;
        if(ftl_map->g_map[victim_seg_idx].table.table_head!=table->table_head){
            printf("error g_map[%d] : %d ,table head : %d   \n",victim_seg_idx,ftl_map->g_map[victim_seg_idx].table.table_head,table->table_head);
            exit(0);
        }
        ftl_map->g_map[victim_seg_idx].table=*table;
        //printf("%d\n",__LINE__);
        //转换为seg
        int seg_num = table2seg(ftl_map->g_map[victim_seg_idx].header_seg.seg,table);
        ftl_map->g_map[victim_seg_idx].header_seg.header = table->table_head;
        //将table_st插入到这个位置
        ...
        if(seg_num>ftl_map->cache->read_cache->max_seg_size)
        {
            ...
            //printf("%d\n",__LINE__);
            //说明插入的是table
            insert_seg_to_read_cache(ssd,&(ftl_map->g_map[victim_seg_idx].table),LRU_TABLE_FLAG);
            //printf("%d\n",__LINE__);
        }
        else
        {
            //printf("%d\n",__LINE__);
            ftl_map->g_map[victim_seg_idx].seg_num = seg_num;
            //说明插入的是段
            insert_seg_to_read_cache(ssd,&(ftl_map->g_map[victim_seg_idx].header_seg),seg_num);
            //printf("%d\n",__LINE__);
        }
...
static uint16_t read_SegTable(struct ssd *ssd, NvmeRequest *req,uint32_t lpn,int* res)
{
    ...
    if(seg_lru[gtd_idx].pos_entry_number==INVALID_POS_ENTRY)
    {
        ...
        //记录读延迟
        struct ppa  tppa;
        tppa = get_gtd_ent_index(ssd, gtd_idx);
    ...
    if(seg_lru[gtd_idx].seg_num==WRITE_CACHE_SPACE)
    {
        ...
        // print_read_cache_LRU(ftl_map);
        // print_write_cache_LRU(ftl_map);
        // print_space(ssd,__LINE__);
        // print_seglru(ssd,__LINE__);
        // print_space_seg(ssd,__LINE__);
    ...
    else
    {
        ...
        else
        {
            ...
            Seg* segs = ((Header_Seg*)(read_cache->read_cache+seg_lru[gtd_idx].pos_entry_number))->seg;
...
static void write_SegTable(struct ssd* ssd,NvmeRequest *req,uint32_t gtd_idx ,uint32_t* lpn,uint16_t* ppn,int num)
{
    FTL_Map* ftl_map = ssd->ftl_map;
    Seg_LRU* seg_lru = ftl_map->seg_LRU;
    Write_Cache* write_cache = ftl_map->cache->write_cache;
    ...
    //获取lpn所在的GTD
    //printf("writeSegTable gtd_idx=%d\n",gtd_idx);
    //获取lpn在GTD中的偏移
    uint32_t offset ;
    uint32_t header;
    uint32_t pos_entry_number;
    G_map* g_map = ftl_map->g_map;
    //printf("write_SegTable\n");
    //printf("%d\n",__LINE__);
    //查看是否在缓存中
    if(seg_lru[gtd_idx].pos_entry_number==INVALID_POS_ENTRY)
    {
        ...
        ppa = get_gtd_ent_index(ssd, gtd_idx);
        

        if (mapped_ppa(&ppa) && valid_ppa(ssd, &ppa)) 
        {
            translation_read_page(ssd, req, &ppa);
            g_map[gtd_idx].next_avail_time = get_lun(ssd, &ppa)->next_lun_avail_time;
        }
        ...
        for(int i = 0;i<num;++i)
        {
            ...
            //更新bitmap
            g_map[gtd_idx].table.bitmap[offset>>5] |= (1<<(offset&31));
            g_map[gtd_idx].table.l2p[offset].vppn = ppn[i];
        }
         //printf("%d\n",__LINE__);
        //printf("write_SegTable1\n");
        //将table插入到write_cache中
         //printf("%d\n",__LINE__);
        insert_table_to_write_table(ssd,&(g_map[gtd_idx].table));
         //printf("%d\n",__LINE__);
        //printf("write_SegTable01\n");
        return;
    }
    else
    {
        //printf("write_SegTable2-1\n");
        //说明在缓存中
        ...
        if(seg_lru[gtd_idx].seg_num==WRITE_CACHE_SPACE)
        {
            ...
            //         int seg_num = table2seg(header_seg.seg,&(write_cache->write_table[pos_entry_number]));
            //         read_SegTable(ssd,req,8);
            //         exit(0);
            //     }
            // }
            //printf("write_SegTable2-1 2\n");
            return;
        }
        else
        {
            ...
            // printf("%d\n",__LINE__);
            //printf("write_SegTable2-2-1\n");
            //将table插入到write_cache中
            insert_table_to_write_table(ssd,&(g_map[gtd_idx].table));
            //printf("write_SegTable2-2-2\n");
            // printf("%d\n",__LINE__);
            return;
        }
        return;
        ...
        if (i == cnt || (lpns[i] >>4)  != current_group) {
            ...
            for (int j = start + 1; j <= i; j++) {
                ...
                if (j == i || lpns[j] != lpns[j-1] + 1 || vppns[j] != vppns[j-1] + 1) {
                    ...
                    //printf("gtd_index:%d current_group:%d  mask %d, ~mask %d \n", gtd_index,current_group ,mask, ~mask );
                    //printf("lr_b->bitmap: %d  \n", lr_b->bitmap );
...
static void batch_gtd_do_gc(struct ssd *ssd, bool force, struct write_pointer *wpp, int num, struct line *delete_line) {
    ...
    // if(cnt<2)
    // 把下边注释掉清除所有的块表明使用全部的块进行GC fullGc
    //cnt = 2;
...
static void gc_read_all_valid_data(struct ssd *ssd, struct ppa *tppa, uint32_t group_gtd_lpns[][256], int *group_gtd_index, int *start_gtd) {
...
static void print_seg_num(struct ssd* ssd,int line)
{
    ...
    G_map* g_map = ssd->ftl_map->g_map;
    bool e = false;
    for(int i = 0;i<ssd->sp.tt_blks;++i)
    {
        if(seg_lru[i].pos_entry_number!=INVALID_POS_ENTRY && seg_lru[i].seg_num !=WRITE_CACHE_SPACE )
        {
            if(seg_lru[i].seg_num!=g_map[i].seg_num)
            {
                printf("erro %d: i: %d gmap_segnum : %d seg_lru_segnum : %d\n",line,i,g_map[i].seg_num,seg_lru[i].seg_num);
                e = true;
            }
        }
    }
...
static void model_training(struct ssd *ssd, struct write_pointer *wpp, uint32_t group_gtd_lpns[][256], int *group_gtd_index, int start_gtd) {
    ...
    Seg_LRU *seg_lru = ftl_map->seg_LRU;
    G_map* g_map = ftl_map->g_map;
    Read_Cache* read_cache = ssd->ftl_map->cache->read_cache;
    Write_Cache* write_cache = ssd->ftl_map->cache->write_cache;
    uint8_t*read_cache_cache = read_cache->read_cache;
    uint32_t gtd_idx;
    //uint64_t train_lpns[parallel][trans_ent];
    uint16_t train_vppns[parallel][trans_ent];
    uint64_t wp_index;
   ...
   //const int interval_size = ssd->sp.interval_size;
    ...
    for (int i = 0; i < parallel; i++) {
        ...
        if(group_gtd_index[i]>0)
        {
            ...
            {
                insert_lr_nodes(ssd,gtd_idx,group_gtd_lpns[i],train_vppns[i],group_gtd_index[i]);
            }    
...
static int batch_line_do_gc(struct ssd* ssd, bool force, struct write_pointer *wpp, struct line *delete_line) {
    ...
    uint32_t group_gtd_lpns[parallel][trans_ent];
...
static uint64_t ssd_write(struct ssd *ssd, NvmeRequest *req)
{
    ...
    if (end_lpn >= spp->tt_pgs) {
        printf("start_lpn=%lld,tt_pgs=%d\n", (long long)end_lpn, ssd->sp.tt_pgs);
        exit(0);
        ftl_err("start_lpn=%"PRIu64",tt_pgs=%d\n", start_lpn, ssd->sp.tt_pgs);
    }
